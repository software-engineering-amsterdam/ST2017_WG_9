...
{--
 create empty environment, anything looking up values will return undefined
 const x _ = x // Always return the first argument
 --}
initEnv :: Env
initEnv = \ _ -> undefined
...
{--
Arrow Free: (easier to reason about)
Prop Name is the atomic form: p
Unary: not p == Neg Form
Cnj [Form]
...
...
instance Show Form where
  show (Prop x)   = show x
  show (Neg f)    = '-' : show f
  show (Cnj fs)     = "AND(" ++ showLst fs ++ ")"
  show (Dsj fs)     = "OR(" ++ showLst fs ++ ")"
