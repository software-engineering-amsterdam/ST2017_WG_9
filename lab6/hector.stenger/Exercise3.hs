...
{- We need to generate composites in a lazy way. Therefor I thought about -}
{- making a generator with a guard that checks for values that are not primes. According -}
{- to wikipedia, https://en.wikipedia.org/wiki/Composite_number, 'so the composite numbers -}
{- are exactly the numbers that are not prime and a unit'. In which a unit is the number 1. -}
composites :: [Integer]
