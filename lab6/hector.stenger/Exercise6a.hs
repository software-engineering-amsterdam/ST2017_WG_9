...
{- Feed the numbers of carmichael into the primeMR function and check what element of carmichael list -}
{- is returned as a false positive. The higher we pick n the higher the false positive is that we find -}
{- . For instance the values of n = 2 are higher then those of n = 1 and n = 3 does not return anything -}
{- because it keeps on running. -}
runner n = do
    hit <- testRunner carmichael n
    return $ elemIndex hit carmichael

{- The k represents the amount of runs a possible false positive gets run. -}
{- The higher number we pick for K the less False positives we find. The increased -}
{- return value represents the higher the false positive is found. Therefor in the -}
{- same range it finds less than a lower value for K would do. -}
{- Only this time the numbers we feed the test function are from the carmichael function -}
{- and not the composite of the primes function. -}
{- However the carmichael value's tend to 'fail' alot more often. This is due to -}
{- the fact that "Fermat's little theorem states that if p is a prime number, then for any integer b,                     -}
{- the number b p − b is an integer multiple of p. Carmichael numbers are composite numbers  -}
{- which have this property." quoted from wikipedia,
- https://en.wikipedia.org/wiki/Carmichael_number -}
testRunner :: [Integer] -> Int -> IO Integer
testRunner (c:cs) k = do
    isPrime <- primeMR k c
    if isPrime then
       return c
    else 
        do
            isSecondPrime <- testRunner cs k
            return isSecondPrime
